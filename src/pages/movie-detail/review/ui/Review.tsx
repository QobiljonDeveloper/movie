...
export const Review: FC = memo(() => {
  ...
  const renderContent = (content: string, reviewId: string) => {
    const words = content.split(" ");
    const isExpanded = expandedIds.includes(reviewId);

    if (words.length <= 50)
      return <p className="dark:text-white text-sy">{content}</p>;

    return (
      <p className="dark:text-white text-sy">
  ...
  if (!reviewData?.results?.length)
    return (
      <div className="container">
        <p className="text-gray-400 dark:text-gray-300">No reviews found.</p>
      </div>
    );
